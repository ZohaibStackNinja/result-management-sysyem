...
export const generateStudentRemarks = async (result: StudentResult): Promise<string> => {
  ...
  if (p >= 95) return `Outstanding achievement, ${firstName}! Your hard work is truly inspiring.`;
  ...
  if (p >= 75) return `Good job, ${firstName}. Consistent effort will take you further.`;
  if (p >= 65) return `Satisfactory progress. Focus on your weaker subjects to improve.`;
  if (p >= 50) return `You passed, but there is significant room for improvement.`;
  if (p >= 40) return `Borderline performance. Extra attention is required in class.`;
  return `Below expectations. Please meet the class teacher for remedial planning.`;
};

export const analyzeClassPerformance = async (results: StudentResult[]): Promise<string> => {
    // Simulate processing
    ...
    if (avg >= 80) {
        ...
        suggestion = "Immediate remedial classes are recommended for core subjects.";
    }
